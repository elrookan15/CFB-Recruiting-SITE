...
export const LeaderboardTop250: React.FC = () => {
  ...
  // ⚡ Bolt Optimization: Memoized recruit filtering
  // Impact: Prevents O(N) array filtering on unrelated state changes (like opening modals), improving render times.
  ...
  // ⚡ Bolt Optimization: Memoized team commits filtering
  // Impact: Prevents unnecessary array filtering when UI state (like modal visibility) updates.
  const teamCommits = useMemo(() => {
    return MOCK_TOP_RECRUITS.filter((r) => r.committedTo?.toLowerCase().includes(selectedTeam.toLowerCase()));
  }, [selectedTeam]);
