...
export const NcaaEligibilityTracker: React.FC = () => {
  ...
  // Consolidated O(N) single-pass calculation memoized for performance
  const stats = useMemo(() => {
    let totalPoints = 0;
    let totalCredits = 0;
    ...
    for (let i = 0; i < courses.length; i++) {
      ...
      // Original logic for completed courses
      if (c.grade !== "In Progress" && c.grade !== "F") {
        completedCount++;
      }

      // Original logic for category requirements (counts "In Progress")
      if (c.grade !== "F") {
      ...
      // Original logic for GPA calculation
    ...
    const calculatedCoreGpa = totalCredits > 0 ? Number((totalPoints / totalCredits).toFixed(2)) : 0;
    const progressPercent = Math.min(100, Math.round((completedCount / 16) * 100));

    const categoryRequirements = [
      { name: "English", required: 4, count: categoryCounts["English"] },
      { name: "Math", required: 3, count: categoryCounts["Math"] },
      { name: "Natural Science", required: 2, count: categoryCounts["Natural Science"] },
      { name: "Social Science", required: 2, count: categoryCounts["Social Science"] },
      { name: "Extra English/Math/Sci", required: 1, count: categoryCounts["Extra English/Math/Sci"] },
      { name: "Additional Core", required: 4, count: categoryCounts["Additional Core"] },
    ];

    return {
      coreGpa: calculatedCoreGpa,
      completedCount,
      progressPercent,
  ...
  const { coreGpa, completedCount, progressPercent, categoryRequirements } = stats;
