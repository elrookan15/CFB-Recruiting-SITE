...
export const CoachesDirectory: React.FC = () => {
  ...
  // ⚡ Bolt Performance Optimization: Memoize the filtered coaches list to prevent unnecessary re-renders
  // on every interaction. Also cache the lowercase search query outside the filter loop.
  const filteredCoaches = useMemo(() => {
    const queryLower = searchQuery.toLowerCase();
    return coaches.filter((coach) => {
      const matchesSearch =
        coach.fullName.toLowerCase().includes(queryLower) ||
        coach.school.toLowerCase().includes(queryLower) ||
        coach.title.toLowerCase().includes(queryLower);
