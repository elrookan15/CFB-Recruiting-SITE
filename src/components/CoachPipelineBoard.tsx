...
export const CoachPipelineBoard: React.FC = () => {
  ...
  // ⚡ Bolt Optimization: Memoize prospect grouping by stage
  // Impact: Prevents O(S*N) array filtering on every render (especially when typing in the Add Prospect modal)
  // groups prospects in a single O(N) pass and recalculates only when prospects array changes.
  const prospectsByStage = useMemo(() => {
    const grouped = {
      Identified: [] as CoachPipelineProspect[],
      Contacted: [] as CoachPipelineProspect[],
      Offered: [] as CoachPipelineProspect[],
      Committed: [] as CoachPipelineProspect[]
    };
    prospects.forEach((p) => {
      if (grouped[p.stage]) {
