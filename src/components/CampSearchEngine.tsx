...
export const CampSearchEngine: React.FC = () => {
  ...
  const filteredCamps = useMemo(() => {
    ...
    const zipQuery = zipCodeFilter.trim();
    return camps.filter((camp) => {
      ...
      if (zipQuery && !camp.zipCode.startsWith(zipQuery)) return false;
      if (query) {
        return (
          camp.name.toLowerCase().includes(query) ||
          camp.host.toLowerCase().includes(query) ||
          camp.city.toLowerCase().includes(query) ||
          camp.state.toLowerCase().includes(query)
        );
      }
      return true;
    });
